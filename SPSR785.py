...
The measurement result will be saved in FILENAME.txt and the measurement parameters in FILENAME_params.txt.
...
parser.add_option("-l", "--location", dest="folder",
                  help="Output location", default="C:/Users/ligo/MIT Dropbox/Dorotea Macri/GRAVITES Measurements/electronics testing/")
...
parser.add_option("-b", "--bandwidth",
                  dest="bandWidth", default="102.4kHz",
...
parser.add_option("-o",
                  dest="order", default=False,
                  action="store_true",
                  help="Channel assignment order: inverts order of window assignment to CH2->displayA CH1->displayB")    
...
time.sleep(0.1)
gpibObj.command('PAUS')  #Pauses to stop data aquisition
time.sleep(0.1)
gpibObj.command('ISRC0') # Input = Analog
time.sleep(0.1)
gpibObj.command('FBAS2,'+str(base)) # Base Frequency = 100kHz
time.sleep(0.1)

#Set up measurement group according to specified channel order
if options.order == True:
    gpibObj.command('ACTD0')
    time.sleep(0.1)
    gpibObj.command('MEAS0,1') #set display 1 = channel2
    time.sleep(0.1)
    gpibObj.command('ACTD1')
    time.sleep(0.1)
    gpibObj.command('MEAS1,0') #set display 2 = channel1
    time.sleep(0.1)
else:
    gpibObj.command('ACTD0')
    time.sleep(0.1)
    gpibObj.command('MEAS0,0') #set display 1 = channel1
    time.sleep(0.1)
    gpibObj.command('ACTD1')
    time.sleep(0.1)
    gpibObj.command('MEAS1,1') #set display 2 = channel2
    time.sleep(0.1)
...
sys.stdout.flush()
print(gpibObj.command('STRT')) #Start measurement
time.sleep(0.1)
...
while measuring:
    #Get status 
    #measuring = not int(gpibObj.query('DSPS?1')).strip()
    response = gpibObj.query('DSPS?1').strip()  # Remove leading/trailing whitespaces
...
if options.plotData:
    import gpibplot
    gpibplot.plotSR785(fpath+d+options.filename,xlog=options.xlog,ylog=options.ylog)
